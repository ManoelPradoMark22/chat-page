...
import { reservationStatusMap } from '@/components/reservation-status'
...
export function stringBRLToNumber(value: string): number {
...
export const returnChangeStatusReservationLabels = (
  status: IReservationStatus,
) => {
  switch (status) {
    case 'pending':
      return 'Aprovar/Cancelar'
    case 'approved':
      return 'Checkout'
    default:
      return undefined
...
export const getTimeDifferenceFormatted = (dateStr: string) => {
  return formatDistanceToNowStrict(dateStr, {
    locale: ptBR,
    addSuffix: true,
...
export const returnDropDownOptionsReservationStatus = () => {
  const dropdownList = [
  ...
  for (const key in reservationStatusMap) {
    dropdownList.push(
      <SelectItem value={key} key={key}>
        {reservationStatusMap[key as IReservationStatus]}
      </SelectItem>,
    )
  }
